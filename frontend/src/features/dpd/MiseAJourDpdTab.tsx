...
export default function MiseAJourDpdTab() {
  ...
  async function charger() {
    ...
    setMessage(null);
    try {
      setAccord(await chargerAccord(c));
      setDernierCode(c);
    } catch (e) {
      setAccord(null);
  ...
  // Auto-chargement dès que la référence est saisie (sortie du champ ou Entrée).
  function chargerSiNouveau() {
    const c = code.trim();
    if (c && c !== dernierCode) charger();
  }
  ...
  async function mettreAJour() {
    ...
    setMessage(null);
    try {
      const res = await synchroniserAccord(code.trim(), commentaire.trim() || undefined);
      setMessage({
        type: 'ok',
        texte: `Dossier mis à jour — version ${res.versionCourante} (historisée, ${res.nbVersions} version(s) au total).`,
      });
    } catch (e) {
  ...
  return (
    <div className="form-card">
      ...
      <div className="loader">
        <div className="field">
          ...
          <input
            ...
            onBlur={chargerSiNouveau}
            onKeyDown={(e) => {
              if (e.key === 'Enter') chargerSiNouveau();
            }}
            placeholder="Ex. ACC-2026-00231"
          />
        </div>
      ...
      {/* Accord chargé — champs en lecture seule, visibles d'emblée puis renseignés. */}
      ...
      <div className="grid cols4">
        ...
        <Ro label="Montant prime" value={accord ? formaterDa(accord.resume.montantPrime) : ''} />
        <Ro label="Statut actuel" value={accord?.resume.statut} />
        <Ro label="Dernière mise à jour" value={accord ? dateLongue(accord.resume.dateDerniereMaj) : ''} span2 />
      </div>

      {/* Nouvel échéancier validé */}
      ...
      {accord && r ? (
        ...
        <div className="ech-info">
          Saisissez le code de l'accord ci-dessus : l'échéancier validé (dates, montants, état de
          règlement) s'affichera ici en lecture seule.
        </div>
      )}
      ...
      <div className="form-actions">
        <button className="btn-primary" onClick={mettreAJour} disabled={sync || !accord}>
      ...
      <div className="note">
        <b>Mise à jour d'un DPD.</b> Quand un nouvel échéancier est validé dans PROASSUR, saisissez
        le <b>code de l'accord</b> : le module récupère le dossier et le nouvel échéancier (lecture
        seule, source PROASSUR) et met à jour le suivi. L'échéancier reste porté par PROASSUR — le
        module ne fait que le refléter ; chaque mise à jour est historisée en version.
      </div>
    </div>
  );
}
